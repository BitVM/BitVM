...
mod tests {
    ...
    use crate::bitvm::model::{
        Paul, PaulCommit, PaulPush, PaulUnlock, Vicky, VickyCommit, VickyPush, VickyUnlock,
    };
    ...
    impl Vicky for DummyVicky {
        ...
        fn next_trace_index(&mut self, _: u8) -> u32 {
            0
        }
        fn trace_challenge(&mut self, _: u8) -> bool {
            false
        }
        ...
        fn next_merkle_index_a(&mut self, _: u8) -> u32 {
            0
        }
        fn next_merkle_index_b(&mut self, _: u8) -> u32 {
            0
        }
        fn next_merkle_index_c_prev(&mut self, _: u8) -> u32 {
            0
        }
        fn merkle_challenge_a(&mut self, _: u8) -> bool {
            false
        }
        fn merkle_challenge_b(&mut self, _: u8) -> bool {
            false
        }
        fn merkle_challenge_c_prev(&mut self, _: u8) -> bool {
            false
        }
        ...
        fn commit(&mut self) -> VickyCommit {
            VickyCommit {
                actor: &mut self.vicky,
            }
        }
        fn push(&mut self) -> VickyPush {
            VickyPush {
                vicky: &mut self.vicky,
            }
        }
