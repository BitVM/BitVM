...
impl TablesVars {
    pub fn new(stack: &mut StackTracker, use_full_tables: bool) -> Self {
        ...
        let half_lookup = if !use_full_tables { u4_push_lookup_table_stack(stack) } else { u4_push_full_lookup_table_stack(stack) };
...
pub fn right_rotate_xored(stack: &mut StackTracker, var_map: &mut HashMap<u8, StackVariable>, x:u8, y:u8, n: u8, tables: &TablesVars ) -> StackVariable {
    let pos_shift = 8 - n / 4;
...
pub fn right_rotate7_xored_sub(stack: &mut StackTracker, mut x : &mut StackVariable, y: StackVariable, tables: &TablesVars, n: u8 ) {
...
pub fn right_rotate7_xored(stack: &mut StackTracker, var_map: &mut HashMap<u8, StackVariable>, x:u8, y:u8, tables: &TablesVars ) -> StackVariable {
...
pub fn g(stack: &mut StackTracker, var_map: &mut HashMap<u8, StackVariable>, a: u8, b: u8, c:u8, d:u8, mx: StackVariable, my: StackVariable, tables: &TablesVars ) {
    ...
    u4_add_stack(stack, 8, 3, vec![vb, mx], vec![&mut va], vec![], tables.quotient, tables.modulo);
    ...
    *va = stack.from_altstack_joined(8, &format!("state_{}",a));
    ...
    u4_add_stack(stack, 8, 2, vec![vd], vec![&mut vc], vec![], tables.quotient, tables.modulo);
    *vc = stack.from_altstack_joined(8, &format!("state_{}",c));
    ...
    let vb = var_map[&b];
    let mut va = var_map.get_mut(&a).unwrap();
    u4_add_stack(stack, 8, 3, vec![vb, my], vec![&mut va], vec![], tables.quotient, tables.modulo);
    *va = stack.from_altstack_joined(8, &format!("state_{}",a));
...
pub fn round(stack: &mut StackTracker, state_var_map: &mut HashMap<u8, StackVariable>, message_var_map: &HashMap<u8, StackVariable>, tables: &TablesVars) {
    g(stack, state_var_map, 0, 4, 8, 12, message_var_map[&0], message_var_map[&1], tables);
    g(stack, state_var_map, 1, 5, 9, 13, message_var_map[&2], message_var_map[&3], tables);
    g(stack, state_var_map, 2, 6, 10, 14, message_var_map[&4], message_var_map[&5], tables);
    g(stack, state_var_map, 3, 7, 11, 15, message_var_map[&6], message_var_map[&7], tables);

    g(stack, state_var_map, 0, 5, 10, 15, message_var_map[&8], message_var_map[&9], tables);
    g(stack, state_var_map, 1, 6, 11, 12, message_var_map[&10], message_var_map[&11], tables);
    g(stack, state_var_map, 2, 7, 8, 13, message_var_map[&12], message_var_map[&13], tables);
    g(stack, state_var_map, 3, 4, 9, 14, message_var_map[&14], message_var_map[&15], tables);
}
...
pub fn compress(stack: &mut StackTracker, chaining: bool, counter: u32, block_len: u32, flags:u32, mut message: HashMap<u8, StackVariable>, tables: &TablesVars, final_rounds: u8, last_round: bool)  {
    ...
    for i in (0..final_rounds).rev() {
        ...
        for n in 0..8 {
            ...
            tmp.push(u4_logic_stack_nib(stack, tables.half_lookup, tables.xor_table, false));
...
pub fn blake3(stack: &mut StackTracker, mut msg_len: u32, final_rounds: u8) {
    ...
    if mandatory_first_block_padding > 0 {
        stack.custom(u4_repeat_number(0, (mandatory_first_block_padding)*2), 0, false, 0, "padding");
    }
    ...
    for i in 0..num_blocks {
        ...
        if last_round && num_padding_bytes > 0 {
            stack.custom(u4_repeat_number(0, (num_padding_bytes)*2), 0, false, 0, "padding");
...
use super::*;
    ...
    fn test_blake3() {
        ...
        stack.custom(script!{ { u4_hex_to_nibbles(&hex_in) } }, 0, false, 0, "msg");
        ...
        stack.custom(script!{ {verify_blake3_hash(hex_out)}}, 1, false, 0, "verify");
    ...
    fn test_g() {
        ...
        let start = stack.get_script_len();
        g(&mut stack, &mut var_map, 0,1,2,3,ret[4],ret[5],&tables);
        let end = stack.get_script_len();
