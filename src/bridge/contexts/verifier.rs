...
use super::base::{BaseContext, generate_keys_from_secret};
...
impl VerifierContext {
  pub fn new(network: Network, n_of_n_secret: &str, operator_public_key: &PublicKey, operator_taproot_public_key: &XOnlyPublicKey, depositor_public_key: &PublicKey, depositor_taproot_public_key: &XOnlyPublicKey, withdrawer_public_key: &PublicKey, withdrawer_taproot_public_key: &XOnlyPublicKey, evm_address: &str) -> Self {
    let (secp, keypair, public_key, taproot_public_key) = generate_keys_from_secret(network, n_of_n_secret);
