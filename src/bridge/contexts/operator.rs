...
impl OperatorContext {
  pub fn new(network: Network, operator_secret: &str, n_of_n_public_key: &PublicKey, n_of_n_taproot_public_key: &XOnlyPublicKey, evm_address: &str) -> Self {
    let (secp, keypair, public_key, taproot_public_key) = generate_keys_from_secret(network, operator_secret);
  ...
  pub fn initialize_withdrawer(&mut self, withdrawer_public_key: &PublicKey, withdrawer_taproot_public_key: &XOnlyPublicKey) {
