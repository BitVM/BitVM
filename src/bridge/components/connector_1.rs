use crate::treepp::*;
use bitcoin::{
    Address, Network, PublicKey, TxIn, Sequence
};
...
impl Connector1 {
    ...
    pub fn generate_script_address(&self) -> Address {
      generate_timelock_script_address(&self.operator_public_key, 2)
    }

    pub fn generate_script_tx_in(&self, input: &Input) -> TxIn {
      let mut tx_in = generate_default_tx_in(input);
      tx_in.sequence = Sequence(u32::try_from(NUM_BLOCKS_PER_WEEK * 2).ok().unwrap() & 0xFFFFFFFF);
      tx_in
  }
}
