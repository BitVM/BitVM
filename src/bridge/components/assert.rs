...
mod tests {
    ...
    use crate::bridge::{
        client::BitVMClient,
        components::{assert::{AssertTransaction, Input}, bridge::BridgeTransaction, connector_b},
    ...
    async fn test_assert_tx() {
        ...
        let funding_utxo = client
            .get_initial_utxo(
                connector_b::generate_taproot_address(&n_of_n_taproot_public_key),
                input_value,
            )
            .await
            .unwrap_or_else(|| {
        ...
        let mut assert_tx = AssertTransaction::new(&context, Input {outpoint: funding_outpoint, amount: input_value});
