const BLAKE3_ZERO_HASHES: [&[u8]; 32] = [
    ...
    &[
        72, 140, 160, 179, 218, 17, 50, 97, 84, 100, 196, 127, 34, 102, 179, 127, 151, 106, 113,
        167,
    ],
    &[
    ...
    &[
        146, 98, 31, 219, 248, 208, 102, 175, 203, 72, 238, 116, 119, 111, 143, 229, 133, 113, 199,
        104,
    ],
    &[
    ...
    &[
        139, 207, 173, 210, 203, 100, 201, 243, 150, 170, 220, 144, 42, 31, 5, 145, 199, 252, 212,
        243,
    ],
    &[
    ...
    &[
        120, 226, 111, 210, 33, 225, 86, 119, 242, 234, 36, 161, 159, 121, 135, 200, 198, 113, 195,
        214,
    ],
    &[
    ...
    &[
        26, 49, 251, 232, 222, 186, 112, 212, 126, 49, 226, 245, 139, 60, 127, 223, 218, 83, 191,
        136,
    ],
    &[
        72, 221, 68, 125, 140, 116, 102, 238, 215, 196, 221, 46, 203, 102, 234, 201, 246, 141, 65,
        60,
    ],
];
...
fn blake3_zero_hash(layer: usize) -> [u8; 20] {
    ...
    zero_node.copy_from_slice(&BLAKE3_ZERO_HASHES[layer][..20]);
    zero_node
}

fn hash(left: [u8; 20], right: [u8; 20]) -> [u8; 20] {
    let mut input = [0u8; 40];
    input[..20].copy_from_slice(&left);
    input[20..(20 + 20)].copy_from_slice(&right);
    let mut hash160 = [0u8; 20];
    let hash = blake3::hash(&input);
    hash160.copy_from_slice(&hash.as_bytes()[..20]);
    hash160
}
