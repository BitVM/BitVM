...
mod test {
    ...
    fn test_mul() {
        ...
        for _ in 0..3 {
            ...
            let script = script! {
                { Fq::push_u32_le_montgomery(&a.to_u32_digits()) }
                { Fq::push_u32_le_montgomery(&b.to_u32_digits()) }
                { Fq::mul() }
                { Fq::push_u32_le_montgomery(&c.to_u32_digits()) }
                { Fq::equalverify(1, 0) }
                OP_TRUE
            };
            let exec_result = execute_script(script);
    ...
    fn test_inv() {
        ...
        for _ in 0..1 {
            ...
            let script = script! {
                { Fq::push_u32_le_montgomery(&BigUint::from(a).to_u32_digits()) }
                { Fq::inv() }
                { Fq::push_u32_le_montgomery(&BigUint::from(c).to_u32_digits()) }
                { Fq::equalverify(1, 0) }
                OP_TRUE
            };
            let exec_result = execute_script(script);
