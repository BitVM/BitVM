...
// input on stack:
// - f            12 elements
// - x', which is a map of point, say -p.x / p.y   1 element
// - y', which is a map of point, say 1 / p.y      1 element
//
...
pub fn collect_line_coeffs(
    ...
    for i in (1..ark_bn254::Config::ATE_LOOP_COUNT.len()).rev() {
        ...
        for j in 0..constants.len() {
            ...
            let double_coeff = constant_iters[j].next().unwrap();
            line_coeff.push(*double_coeff);
            // add line coeff
            if ark_bn254::Config::ATE_LOOP_COUNT[i - 1] == 1
                || ark_bn254::Config::ATE_LOOP_COUNT[i - 1] == -1
            {
                let add_coeff = constant_iters[j].next().unwrap();
                line_coeff.push(*add_coeff);
            }
