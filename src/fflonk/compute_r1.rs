...
mod test {
    ...
    fn test_compute_r1() {
        ...
        let script = script! {
            ...
            // push pi, zhInv
            { Fr::push_dec_montgomery("12368363170870087162509434874521168463460384615249055347885673275750149676873") }
            ...
            // push LiS1Inv 1-4
            { Fr::push_u32_le_montgomery(&BigUint::from(lis1_1_inv).to_u32_digits()) }
            { Fr::push_u32_le_montgomery(&BigUint::from(lis1_2_inv).to_u32_digits()) }
            { Fr::push_u32_le_montgomery(&BigUint::from(lis1_3_inv).to_u32_digits()) }
            { Fr::push_u32_le_montgomery(&BigUint::from(lis1_4_inv).to_u32_digits()) }
