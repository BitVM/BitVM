...
use bitvm::{
    self,
    bridge::{
        client::BitVMClient, components::{connector_a::ConnectorA, connector_b::ConnectorB, connector_c::ConnectorC, connector_z::ConnectorZ}, context::BridgeContext, graph::{DEPOSITOR_SECRET, EVM_ADDRESS, N_OF_N_SECRET, OPERATOR_SECRET, WITHDRAWER_SECRET}
    },
};

pub fn setup_test() -> (BitVMClient, BridgeContext, ConnectorA, ConnectorB, ConnectorC, ConnectorZ) {
    ...
    let connector_z = ConnectorZ::new(context.network, context.evm_address.as_ref().unwrap(), &context.depositor_taproot_public_key.unwrap(), &context.n_of_n_taproot_public_key.unwrap());

    return (client, context, connector_a, connector_b, connector_c, connector_z);
}
