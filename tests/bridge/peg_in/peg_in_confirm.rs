...
use bitvm::bridge::{
    components::{
        bridge::BridgeTransaction, connector_z::ConnectorZ, helper::*, peg_in_confirm::PegInConfirmTransaction
    },
...
async fn test_peg_in_confirm_tx() {
    ...
    let connector_z = ConnectorZ::new(context.network, &evm_address, &context.depositor_taproot_public_key.unwrap(), &context.n_of_n_taproot_public_key.unwrap());
